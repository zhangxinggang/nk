...
module.exports = function (sender) {
  /**
     ...
     */
  var formLiveParam = require(pathModule.join(
    NKGlobal.config.httpServer.httpServerPath,
    "servers/video/formLiveParam.js"
  ));
  var pushers = {};
    ...
    if (session.isStarting) {
      ...
      if (!session.m3u8Path && session.isPublishing) {
        ...
        var m3u8Path =
          sender.req.protocol +
          "://" +
          sender.req.headers.host +
          formLiveParam.liveAddr(currentStream);
        currentStream.tempLive = true;
        var LiveStream = require(pathModule.join(
          NKGlobal.config.httpServer.httpServerPath,
          "servers/video/liveStream.js"
        ));
