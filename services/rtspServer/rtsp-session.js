...
class RTSPSession extends event.EventEmitter {
  ...
  *genHandleData() {
        ...
        if (mtcp) {
          ...
          if (control == this.aControl) {
            ...
            if (this.aRTPControlClientPort) {
              this.aRTPControlClientSocket = dgram.createSocket(
                this.getUDPType()
              );
            }
            if (this.type == "pusher") {
              ...
              await this.bindUDPPort(
                this.aRTPServerSocket,
                this.aRTPServerPort
              );
              this.aRTPControlServerPort = await getPort();
              this.aRTPControlServerSocket = dgram.createSocket(
                this.getUDPType()
              );
          ...
          if (control == this.vControl) {
            ...
            if (this.vRTPControlClientPort) {
              this.vRTPControlClientSocket = dgram.createSocket(
                this.getUDPType()
              );
            }
            if (this.type == "pusher") {
              ...
              this.vRTPControlserverSokcet = dgram.createSocket(
                this.getUDPType()
              );
              ...
              await this.bindUDPPort(
                this.vRTPControlserverSokcet,
                this.vRTPControlServerPort
              );
              ts = ts.split(";");
              ts.splice(
                ts.indexOf(mudp[0]) + 1,
  ...
  stop() {
    ...
    console.log(
      `rtsp session[type=${this.type}, path=${this.path}, id=${this.id}] end`
    );
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (var rtpBuf of this.pushSession.gopCache) {
        if (this.transType == "tcp") {
          ...
          await this.sendUDPPack(
            rtpBuf,
            this.vRTPClientSocket,
            this.vRTPClientPort,
            this.host
          );
  ...
  sendVideoControl(rtpBuf) {
    if (this.transType == "tcp") {
      ...
      this.vRTPControlClientSocket.send(
        rtpBuf,
        this.vRTPControlClientPort,
        this.host
      );
      this.outBytes += rtpBuf.length;
      this.pushSession.outBytes += rtpBuf.length;
  ...
  sendAudioControl(rtpBuf) {
    if (this.transType == "tcp") {
      ...
      this.aRTPControlClientSocket.send(
        rtpBuf,
        this.aRTPControlClientPort,
        this.host
      );
      this.outBytes += rtpBuf.length;
      this.pushSession.outBytes += rtpBuf.length;
