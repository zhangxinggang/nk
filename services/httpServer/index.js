...
class HttpServer {
  ...
  start(callback) {
    ...
    if (conf_https && conf_https.start) {
      services.push(
        new Promise((resolve, reject) => {
          ...
          let options = {
            key:
              conf_https.key.value ||
              (conf_https.key.path && fs.readFileSync(conf_https.key.path)) ||
              private_pkcs1,
            cert:
              conf_https.cert.value ||
              (conf_https.cert.path && fs.readFileSync(conf_https.cert.path)) ||
              certificate,
          };
          https
            .createServer(options, app.callback())
            .listen(conf_https.port, () => {
    ...
    Promise.all(services).then(() => {
      console.info(
        `httpServer info : PID:${process.pid}  NODE_ENV : ${app.env}`
      );
      callback && callback();
    });
  }
